...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_encoder(self, x, timestamps, mask_ratio, mask=None):
        ...
        pos_embed_spatial = get_2d_sincos_pos_embed(
            embed_dim = x_emb.shape[2] - 384,
            grid_size = grid_size,
            cls_token=False
        )
        pos_embed_spatial = torch.from_numpy(pos_embed_spatial).to(x.device).float()
        pos_embed_spatial = pos_embed_spatial.repeat(T,1).unsqueeze(0).expand(B,-1,-1)

        timestamps_flat = timestamps.reshape(B*T,3)
        ts_embed_list = [
            get_1d_sincos_pos_embed_from_grid_torch(128, timestamps_flat[:,k].float())
            for k in range(3)
        ]
    ...
    def forward_decoder(self, x, timestamps, ids_restore):
        ...
        timestamps_flat = timestamps.reshape(B*T,3)
        ts_embed_list = [
            get_1d_sincos_pos_embed_from_grid_torch(64, timestamps_flat[:,k].float())
            for k in range(3)
        ]
        ts_embed = torch.cat(ts_embed_list, dim=1)
        ts_embed = ts_embed.reshape(B,T,1,-1).expand(B,T,L_per_step,-1).reshape(B,T*L_per_step,-1)
...
if __name__ == "__main__":
    ...
    timestamps = torch.randint(low=0, high=1000, size=(B,T,3))
    ...
    loss, pred, mask = model(imgs, timestamps, mask_ratio=0.75)
